#!/usr/bin/env bash
# Build and push the Search API image to ECR.
set -euo pipefail

AWS_PROFILE="${AWS_PROFILE:-sandbox-sso}"
AWS_REGION="${AWS_REGION:-ca-central-1}"
ACCOUNT_ID="${ACCOUNT_ID:-472730591037}"
IMAGE_NAME="${IMAGE_NAME:-arxplorer-search}"
...
echo "Building ${ECR_REPO}:${IMAGE_TAG} from ${BUILD_CONTEXT} (Dockerfile=${DOCKERFILE})..."
